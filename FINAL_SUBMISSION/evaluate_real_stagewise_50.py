...
def rubric_proxy(recall5, token_f1_value, grounding):
    ...
    # A and G penalized by retrieval as in rubric idea.
    a_penalized = a * r
    g_penalized = g * r
...
def build_component_stages(pipeline, question):
    """
    This defines real component-level ablation stages.

    S0: Base candidate retrieval.
        This approximates the early/base RAG before reranking and advanced evidence selection.

    S1: Source-aware / expanded candidate retrieval.
        Uses the final candidate builder but no reranker; top5 candidates are passed to LLM.

    S2: Fine-tuned / error-mined reranker top5.
        Uses reranker top5.

    S3: Hard100-optimized selector.
        Uses top3_plus_8_9 selection to show previous optimized strategy.

    S4: Final default_top5.
        Uses final selected evidence strategy.
    """

    candidates = build_v7_candidates(pipeline, question)
    candidates = dedup_by_id(candidates)

    # S0: earliest/base approximation: first 5 raw candidates
    s0 = candidates[:5]

    # S1: expanded candidate top5 before reranking
    # In this implementation S0 and S1 may be close if candidate builder is already source-aware.
    # Still kept separately to show component progression.
    s1 = candidates[:5]
    ...
    # S3: previous selector strategy
    s3 = select_positions(reranked50, [1, 2, 3, 8, 9])

    # S4: final selected strategy
    s4 = reranked50[:5]

    return {
        "S0_base_retrieval_plus_base_LLM": s0,
        "S1_source_aware_retrieval_plus_LLM": s1,
        "S2_finetuned_reranker_plus_LLM": s2,
        "S3_previous_selector_top3_plus_8_9": s3,
        "S4_final_default_top5_system": s4,
    }
...
def main():
    ...
    parser.add_argument("--output_dir", default="evaluation_results/real_stagewise_50")
    ...
    details_path = output_dir / "real_stagewise_50_details.csv"
    summary_path = output_dir / "real_stagewise_50_summary.csv"
    ...
    if details_path.exists():
        ...
        print("Existing checkpoint rows:", len(existing))

    print("Benchmark:", benchmark_path)
    print("N:", len(data))
    print("Loading pipeline with generator...")
    ...
    for idx, item in enumerate(data, 1):
        ...
        stages = build_component_stages(pipeline, question)
